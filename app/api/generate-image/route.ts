...
async function inferImageSize(imageBuffer: Buffer): Promise<EditImageSize> {
  try {
    ...
    if (aspectRatio > 1.08) return "1024x1536"; // this means that the input image is wider than it is tall
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (APICallError.isInstance(error)) {
      return NextResponse.json(
        { eror: error.message || "Image generation failed. Please try again." },
